...
class Ak01PdfController extends Controller
{
    public function generateAk01(Request $request, $id_sertifikasi)
    {
        ...
        $tanggalRespon = $responBukti->first() ? Carbon::parse($responBukti->first()->created_at)->isoFormat('dddd, DD MMM YYYY') : Carbon::now()->isoFormat('dddd, DD MMM YYYY');

        // Format tanggal pelaksanaan
        $tanggalPelaksanaan = $jadwal->tanggal_pelaksanaan ? Carbon::parse($jadwal->tanggal_pelaksanaan) : Carbon::now();

        $hari = $tanggalPelaksanaan->isoFormat('dddd');
        $tanggal = $tanggalPelaksanaan->isoFormat('D MMMM Y');

        // Format waktu mulai
        $waktuMulai = $jadwal->waktu_mulai ? Carbon::parse($jadwal->waktu_mulai)->format('H:i') : '00:00';
