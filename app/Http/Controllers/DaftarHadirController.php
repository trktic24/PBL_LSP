...
{  
    ...
    public function index(Request $request, $id_jadwal)
    {
        ...
        $query = DataSertifikasiAsesi::query()
                    ->with(['asesi.user', 'asesi.pekerjaan'])
        ...
        return view('frontend.daftar_hadir', [
            'jadwal' => $jadwal,
            'pendaftar' => $pendaftar,
            'perPage' => $perPage,
    ...
    public function simpan(Request $request, $id_jadwal)
    {
        $data = json_decode($request->data_presensi, true);

        foreach ($data as $p) {

            $asesi = DataSertifikasiAsesi::where('id_data_sertifikasi_asesi', $p['id_data_sertifikasi_asesi'])
                    ->with('asesi')
                    ->first();

            DaftarHadirAsesi::create([
                'id_data_sertifikasi_asesi' => $p['id_data_sertifikasi_asesi'],
                'status_hadir' => $p['hadir'],
                'tanda_tangan_asesi' => $p['hadir'] ? $asesi->asesi->tanda_tangan : null
            ]);
        }
