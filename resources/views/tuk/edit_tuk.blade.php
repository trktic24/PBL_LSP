...
<head>
  ...
  <title>Edit TUK | LSP Polines</title>
    ...
    <main class="flex-1 flex justify-center items-start pt-10 pb-12">
      <div class="w-full max-w-4xl bg-white border border-gray-200 rounded-xl shadow-lg p-10">
        ...
        <div class="flex items-center justify-between mb-10">
          ...
          <h1 class="text-3xl font-bold text-gray-900 text-center flex-1">EDIT TUK</h1>
          <div class="w-[80px]"></div>
        </div>

        <form action="{{ route('update_tuk', $tuk->id_tuk) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
          @csrf
          @method('PATCH')
          ...
          <div>
            ...
            <input type="text" id="nama_lokasi" name="nama_lokasi" required
              ...
              value="{{ old('nama_lokasi', $tuk->nama_lokasi) }}" />
          </div>
          ...
          <div>
            ...
            <textarea id="alamat_tuk" name="alamat_tuk" rows="3" required
              class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
              placeholder="Masukkan Alamat Lengkap TUK">{{ old('alamat_tuk', $tuk->alamat_tuk) }}</textarea>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" id="kontak_tuk" name="kontak_tuk" required
                ...
                value="{{ old('kontak_tuk', $tuk->kontak_tuk) }}" />
            </div>
            ...
            <div x-data="{ fileName: '' }">
              ...
              <label class="w-full flex items-center px-4 py-3 bg-white border border-gray-300 rounded-lg shadow-sm cursor-pointer hover:bg-gray-50 focus-within:ring-2 focus-within:ring-blue-500">
                ...
                <span x-text="fileName || 'Ganti foto TUK (biarkan jika tidak diubah)...'" class="text-sm text-gray-600"></span>
                <input type="file" name="foto_tuk" @change="fileName = $event.target.files[0].name" class="opacity-0 absolute w-0 h-0" />
              </label>
