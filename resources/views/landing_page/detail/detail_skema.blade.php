...
@section('content')
    ...
    <section class="container mx-auto px-8 mt-20">
        <div class="relative h-[500px] rounded-[2rem] overflow-hidden shadow-xl">
            ...
            <img src="{{ asset('images/' . ($skema->gambar ?? 'placeholder_default.jpg')) }}" alt=""
            ...
            <div class="absolute inset-0 flex flex-col justify-center px-12 text-white">
                <h1 class="text-6xl font-bold mb-4">
                    {{ strtoupper($skema->nama_skema ?? ($skema->nama ?? 'Skema Sertifikasi')) }}</h1>
        ...
        <div class="max-w-3xl mx-auto space-y-4">
            @forelse($skema->jadwals as $jadwal)
                <div class="bg-white rounded-2xl shadow-[6px_6px_20px_rgba(0,0,0,0.25)] p-8 border-2 border-blue-500">
                    ...
                    <h3 class="font-bold text-xl text-center mb-6">{{ strtoupper($skema->nama_skema ?? 'Jadwal Skema') }}
                    </h3>
                    ...
                    <div class="space-y-4 mb-8">
                        ...
                        <div class="flex items-center gap-3 text-base">
                            ...
                            <span>Waktu: {{ $jadwal->tanggal_mulai ? $jadwal->tanggal_mulai->format('H:i') : 'N/A' }} -
                                {{ $jadwal->tanggal_selesai ? $jadwal->tanggal_selesai->format('H:i') : 'N/A' }}</span>
                        </div>
                    </div>
                ...
                @foreach ($skema->skkni as $skkni)
                    <div class="bg-blue-50 rounded-2xl shadow-md p-4 flex items-center justify-between">
                        ...
                        <a href="{{ $skkni->link_pdf ?? '#' }}"
                            class="text-blue-600 hover:text-blue-800 font-semibold text-sm">Lihat PDF</a>
                    </div>
                @endforeach
            @else
