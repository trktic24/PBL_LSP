...
<main class="main-content p-6 sm:p-10"> 
    ...
    @if($mode === 'edit')
    <form class="form-body mt-6" action="{{ route('ia09.store') }}" method="POST" id="formIA09"> 
        @csrf 
    @else
                ...
                <tbody class="bg-white"> 
                    ...
                    <tr> 
                        ...
                        <td class="border border-gray-900 px-3 py-1 text-xs align-top">
                            {{ $item['pertanyaan'] }}
                            @if($mode === 'edit')
                            <input type="hidden" name="pertanyaan[{{ $item['no'] }}][pertanyaan]" value="{{ $item['pertanyaan'] }}">
                            <input type="hidden" name="pertanyaan[{{ $item['no'] }}][no]" value="{{ $item['no'] }}">
                            @if($item['id_jawaban'])
                            <input type="hidden" name="pertanyaan[{{ $item['no'] }}][id_jawaban]" value="{{ $item['id_jawaban'] }}">
                            @endif
                            @endif
                        </td> 
                        ...
                        <td class="border border-gray-900 px-3 py-1 text-xs"> 
                            @if($mode === 'edit')
                                <textarea 
                                    name="pertanyaan[{{ $item['no'] }}][jawaban]" 
                                    rows="4" 
                                    required
                                    placeholder="Tulis kesimpulan jawaban asesi (minimal 10 karakter)..."
                                    class="form-textarea w-full border-gray-300 rounded-md p-2 text-xs focus:border-blue-500 focus:ring-blue-500 @error('pertanyaan.'.$item['no'].'.jawaban') border-red-500 @enderror">{{ old('pertanyaan.'.$item['no'].'.jawaban', $item['jawaban']) }}</textarea>
                                @error('pertanyaan.'.$item['no'].'.jawaban')
                        ...
                        <td class="border border-gray-900 text-center"> 
                            <input 
                                type="radio" 
                                name="pertanyaan[{{ $item['no'] }}][pencapaian]" 
                                value="Ya" 
                                id="pencapaian_ya_{{ $item['no'] }}" 
                                {{ old('pertanyaan.'.$item['no'].'.pencapaian', $item['pencapaian']) == 'Ya' ? 'checked' : '' }} 
                                @if($mode === 'edit') required @else disabled @endif
                                class="form-radio h-4 w-4 text-green-600 border-gray-300 focus:ring-green-500"
                                title="Pencapaian Ya"> 
                            <label for="pencapaian_ya_{{ $item['no'] }}" class="sr-only">Ya</label> 
                        </td> 
                        ...
                        <td class="border border-gray-900 text-center"> 
                            <input 
                                ...
                                name="pertanyaan[{{ $item['no'] }}][pencapaian]" 
                                value="Tidak" 
                                id="pencapaian_tidak_{{ $item['no'] }}" 
                                {{ old('pertanyaan.'.$item['no'].'.pencapaian', $item['pencapaian']) == 'Tidak' ? 'checked' : '' }} 
                                @if($mode === 'edit') required @else disabled @endif
                                class="form-radio h-4 w-4 text-red-600 border-gray-300 focus:ring-red-500"
                                title="Pencapaian Tidak"> 
                            <label for="pencapaian_tidak_{{ $item['no'] }}" class="sr-only">Tidak</label> 
                        </td> 
                    </tr> 
                    @endforeach 
                </tbody> 
