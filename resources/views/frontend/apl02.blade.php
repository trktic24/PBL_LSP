    ...
    <div class="p-4 md:p-8">
        ...
        @php
            ...
            foreach ($skema->kelompokPekerjaan as $kelompok) {
                ...
                Log::info($kelompok->unitKompetensi->toArray());
            }
        @endphp
        ...
        @else
            ...
            @else
                @if ( $sertifikasi->rekomendasi_apl02 !== null)
                    <div class="text-gray-600 mb-8 p-4 bg-green-50 rounded-lg border-l-4 border-green-400">
                        APL-02 <strong>telah diverifikasi</strong> Asesor.
                    </div>
                @else
                    <div class="text-gray-600 mb-8 p-4 bg-yellow-50 rounded-lg border-l-4 border-yellow-400">
                        APL-02 <strong>belum diverifikasi</strong> Asesor. Asesor perlu melakukan <strong>verifikasi</strong> APL-02 agar Asesi dapat melanjutkan proses asesmen.
                    </div>
                @endif
            @endif    
            ...
            <form id="apl02-form" method="POST" action="{{ isset($isMasterView) ? '#' : route('api.v1.apl02.store', ['id_sertifikasi' => $idDataSertifikasi]) }}" enctype="multipart/form-data">
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 flex justify-between items-center pb-8">
                    ...
                    @else
                        <button type="button" id="btn-verifikasi" 
                                data-action="verifikasi"
                                class="w-48 py-3 bg-blue-500 text-white font-bold rounded-full shadow-lg hover:bg-blue-600 transition duration-200 focus:outline-none focus:ring-4 focus:ring-blue-300 transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed text-center"
                                {{ $sertifikasi->rekomendasi_apl02 === 'diterima' ? 'disabled' : '' }}>
                            Verifikasi
                        </button>
                    @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const statusSpan = document.getElementById('verifikasi-status');
    const csrfToken = document.querySelector('meta[name="csrf-token"]').content;

    if (btnVerifikasi) {
        btnVerifikasi.addEventListener('click', function() {
            Swal.fire({
                title: 'Verifikasi APL-02?',
                text: "Data APL-02 akan diverifikasi dan tidak bisa diubah kembali!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await fetch("{{ route('asesor.apl02.verifikasi', $sertifikasi->id_data_sertifikasi_asesi) }}", {
                            method: 'POST',
                            headers: {
                        ...
                        if (data.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil!',
                                text: data.message || "APL-02 berhasil diverifikasi.",
                                timer: 2000,
                            ...
                            // 2. Update keterangan
                            if (statusSpan) {
                                statusSpan.innerText = "APL-02 sudah diverifikasi";
                            }
                        } else {
