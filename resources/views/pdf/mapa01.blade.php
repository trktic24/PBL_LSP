...
<body>
    ...
    <table class="no-border" style="margin-bottom: 15px;">
        <tr>
            ...
            <td width="10">:</td>
            <td>
                <strong>Judul:</strong> {{ $sertifikasi->jadwal->skema->nama_skema ?? 'Junior Web Programmer' }} <br>
                <strong>Nomor:</strong> {{ $sertifikasi->jadwal->skema->nomor_skema ?? '-' }}
            </td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            <td rowspan="2" width="5%" class="text-center font-bold">1.1</td>
            <td rowspan="2" width="20%" class="font-bold">Asesi</td>
            <td>
                ...
                Hasil pelatihan / pendidikan (Kurikulum telusur)
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            <td rowspan="2"></td>
            <td rowspan="2" class="font-bold">Konteks Asesmen</td>
            <td>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th rowspan="2" width="20%">Bukti-Bukti</th>
                <th colspan="3">Jenis Bukti</th>
                <th colspan="3">Metode</th>
            </tr>
            <tr>
                <th width="5%">L</th>
                <th width="5%">TL</th>
                ...
                <th width="10%">Tes Tulis</th>
                <th width="10%">Tes Lisan</th>
            </tr>
        </thead>
        <tbody>
            @if(isset($mapa01->unit_kompetensi) && count($mapa01->unit_kompetensi) > 0)
                ...
                <tr>
                    <td>{{ $idx + 1 }}. {{ $u['judul_unit'] ?? 'Unit ' . ($idx+1) }}</td>
                    <td>{{ $u['bukti'] ?? '-' }}</td>
                    ...
                    <td class="text-center check-mark">{{ isset($u['tes_tulis']) ? 'V' : '' }}</td> {{-- Sesuaikan nama key --}}
                    <td class="text-center check-mark">{{ isset($u['tes_lisan']) ? 'V' : '' }}</td> {{-- Sesuaikan nama key --}}
                </tr>
                @endforeach
            @else
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td class="text-center font-bold">PENYUSUN</td>
                <td>{{ $mapa01->penyusun[0]['nama'] ?? '-' }}</td>
                <td>{{ $mapa01->penyusun[0]['ttd_tanggal'] ?? '-' }}</td>
            </tr>
            <tr>
                <td class="text-center font-bold">VALIDATOR</td>
