...
<head>
  ...
  <style>
    ...
    /* ANIMASI FORGOT PASS */
    /* Gambar masuk dari Kanan (seolah bekas posisi Login) */
    @keyframes imageEnterFromRight {
        0% { transform: translateX(100%); opacity: 0; }
        100% { transform: translateX(0); opacity: 1; }
    }

    /* Form masuk dari Kiri */
    @keyframes formEnterFromLeft {
        0% { transform: translateX(-100%); opacity: 0; }
        100% { transform: translateX(0); opacity: 1; }
    }

    .animate-image-entry {
        animation: imageEnterFromRight 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards;
    }
    
    .animate-form-entry {
        animation: formEnterFromLeft 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards;
    }
  </style>
</head>
...
<body class="bg-white overflow-hidden">
  ...
  <main class="flex h-screen w-full">
    ...
    <div class="hidden lg:block w-1/2 h-full relative bg-cover bg-center select-none 
                rounded-tr-[4%] rounded-br-[4%] 
                shadow-[10px_0_30px_rgba(0,0,0,0.4)] 
      ...
      <div class="relative text-white px-10 mt-8 z-10">
        <h1 class="text-[42px] font-bold leading-tight drop-shadow-2xl">
          Don't be Afraid if you Forgot<br>
          your Password!
        </h1>
      </div>
      ...
      <div class="absolute bottom-20 left-1/2 -translate-x-1/2 w-[95%] flex items-center justify-between text-white z-10">
        ...
        <button id="prevBtn" class="group p-2 rounded-full hover:bg-white/10 transition-all duration-700 focus:outline-none active:scale-90">
        ...
        <button id="nextBtn" class="group p-2 rounded-full hover:bg-white/10 transition-all duration-700 focus:outline-none active:scale-90">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"
            class="w-8 h-8 text-white/70 group-hover:text-white transition-colors">
    ...
    <div class="w-full lg:w-1/2 h-full bg-white flex flex-col justify-center px-10 lg:px-20 relative select-none z-10 overflow-hidden">
      ...
      <div class="form-container mx-auto shadow-2xl relative z-20">
        ...
        @if(session('status'))
        <div class="mb-4 p-3 bg-green-100 border border-green-400 text-green-700 text-xs rounded-lg">
          {{ session('status') }}
        </div>
        @endif
        ...
        <form action="{{ route('forgot_pass.send') }}" method="POST" class="space-y-6">
          ...
          <button type="submit"
            class="w-full bg-[#0D63F3] text-white py-3 rounded-xl text-[15px] font-semibold shadow-lg hover:bg-blue-700 hover:shadow-xl active:scale-[0.98] transition-all duration-200">
            Kirim Link Reset
          </button>
        </form>
        ...
        <div class="mt-6 text-center text-sm">
          ...
          <a href="{{ route('login_admin') }}" class="text-blue-600 hover:text-blue-800 font-medium transition-colors ml-1">
            Login disini
          </a>
        </div>
      ...
      <img src="{{ asset('images/pixel2.png') }}" class="pixel2"
        style="--px-size: 250px; --px-bottom: -50px; --px-left: -13px;">
