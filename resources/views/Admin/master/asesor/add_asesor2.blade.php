          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" id="nomor_regis" name="nomor_regis" required
                     ...
                     placeholder="Masukkan No Registrasi" value="{{ old('nomor_regis', $asesor->nomor_regis ?? '') }}">
            </div>
            <div>
              ...
              <input type="text" id="nama_lengkap" name="nama_lengkap" required
                     class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                     placeholder="Masukkan Nama Lengkap" value="{{ old('nama_lengkap', $asesor->nama_lengkap ?? '') }}">
            </div>
            <div class="md:col-span-2">
              <label for="nik" class="block text-sm font-medium text-gray-700 mb-2">NIK <span class="text-red-500">*</span></label>
              <input type="text" id="nik" name="nik" required
                     class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                     placeholder="Masukkan NIK" value="{{ old('nik', $asesor->nik ?? '') }}">
            </div>
          </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-3 gap-4">
                <select id="tanggal_lahir" name="tanggal_lahir" required
                  ...
                  @for ($i = 1; $i <= 31; $i++)
                    <option value="{{ $i }}" {{ old('tanggal_lahir', $asesor->tanggal_lahir ?? '') == $i ? 'selected' : '' }}>{{ $i }}</option>
                  @endfor
                </select>
                <select id="bulan_lahir" name="bulan_lahir" required
                  ...
                  <option value="">Bulan</option>
                   @foreach (['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'] as $key => $bulan)
                    <option value="{{ $key + 1 }}" {{ old('bulan_lahir', $asesor->bulan_lahir ?? '') == ($key + 1) ? 'selected' : '' }}>{{ $bulan }}</option>
                  @endforeach
                </select>
                <select id="tahun_lahir" name="tahun_lahir" required
                  ...
                  @for ($i = date('Y'); $i >= date('Y') - 70; $i--)
                    <option value="{{ $i }}" {{ old('tahun_lahir', $asesor->tahun_lahir ?? '') == $i ? 'selected' : '' }}>{{ $i }}</option>
                  @endfor
                </select>
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" id="pekerjaan" name="pekerjaan" required
                     class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                     placeholder="Masukkan Pekerjaan" value="{{ old('pekerjaan', $asesor->pekerjaan ?? '') }}">
            </div>
