...
@section('title', 'Daftar Jadwal - Template ' . ($formConfig['label'] ?? 'Formulir'))
...
<div class="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-10 border border-gray-100 min-h-full my-4">
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Pilih Jadwal & Asesor</h1>
            <p class="text-blue-600 font-semibold uppercase tracking-wider text-sm mt-1">
                {{ $skema->nomor_skema }} - {{ $formConfig['label'] ?? $form_code }} ({{ $formConfig['name'] ?? 'Master Template' }})
            </p>
        </div>
    ...
    @php
        // Transform FR.IA.05 -> ia05
        $suffix = strtolower(str_replace(['FR.', '.'], '', $form_code));
        $templateRoute = "admin.skema.template." . $suffix;
    @endphp
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-blue-50/40 transition group">
                    ...
                    <td class="px-6 py-6 text-center">
                        {{-- Note: Master templates currently only take id_skema, but picking a Jadwal fulfills the desired flow --}}
                        <a href="{{ route($templateRoute, ['id_skema' => $skema->id_skema, 'id_jadwal' => $jadwal->id_jadwal]) }}" class="inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl transition shadow-sm hover:shadow-blue-200 transform hover:scale-105 active:scale-95 whitespace-nowrap">
