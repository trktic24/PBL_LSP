        ...
        @if ($sertifikasi)
            ...
            <aside
                ...
                <div class="text-center mb-6">
                    ...
                    <p class="text-gray-600 text-sm mt-2">{{ $sertifikasi->jadwal->skema->nomor_skema ?? 'Nomor Skema' }}
                    </p>
                ...
                <div class="max-w-3xl mx-auto bg-white p-8 sm:p-12 rounded-2xl shadow-xl">
                    ...
                    <ol class="relative">
                        ...
                        @php
                            ...
                            // --- Variabel Level Biar Gampang Dibaca ---
                            $level = $sertifikasi->progres_level;
                            
                            $level_daftar = DataSertifikasiAsesi::STATUS_PENDAFTARAN_SELESAI;
                            $level_tunggu_bayar = DataSertifikasiAsesi::STATUS_TUNGGU_VERIFIKASI_BAYAR;
                            $level_lunas = DataSertifikasiAsesi::STATUS_PEMBAYARAN_LUNAS;
                            $level_pra_asesmen = DataSertifikasiAsesi::STATUS_PRA_ASESMEN_SELESAI;
                            $level_setuju = DataSertifikasiAsesi::STATUS_PERSETUJUAN_ASESMEN_OK;
                            $level_asesmen = DataSertifikasiAsesi::STATUS_ASESMEN_LISAN_SELESAI; // Kita anggap lisan yg terakhir
                            $level_umpan_balik = DataSertifikasiAsesi::STATUS_UMPAN_BALIK_SELESAI;
                            $level_rekomendasi = DataSertifikasiAsesi::STATUS_DIREKOMENDASIKAN;
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_daftar) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                ...
                                <a href="{{ route('apl01.download', ['id_asesi' => $sertifikasi->id_asesi]) }}" target="_blank"
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_lunas) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                {{-- Link: Bisa diklik jika sudah selesai daftar (level 10) --}}
                                @if ($level >= $level_daftar)
                                ...
                                @elseif ($level == $level_tunggu_bayar)
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_pra_asesmen) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                ...
                                {{-- Status Dinamis --}}
                                @if ($level >= $level_pra_asesmen)
                                    <p class="{{ $statusClassSelesai }}">Selesai</p>
                                    <button class="mt-2 px-4 py-1.5 bg-blue-500 text-white text-xs font-semibold rounded-md hover:bg-blue-600">
                                        Unduh Dokumen
                                    </button>
                                @elseif ($level == $level_lunas)
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_setuju) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                {{-- Link: Bisa diklik jika sudah selesai Pra-Asesmen (level 40) --}}
                                @if ($level >= $level_pra_asesmen)
                                    <a href="{{ route('kerahasiaan.fr_ak01', ['id_asesi' => $sertifikasi->id_asesi]) }}" class="{{ $linkClassEnabled }}">
                                ...
                                {{-- Status Dinamis --}}
                                @if ($level >= $level_setuju)
                                    <p class="{{ $statusClassSelesai }}">Telah Disetujui</p>
                                    <button class="mt-2 px-4 py-1.5 bg-blue-500 text-white text-xs font-semibold rounded-md hover:bg-blue-600">
                                        Unduh Dokumen
                                    </button>
                                @elseif ($level == $level_pra_asesmen)
                                    <p class="{{ $statusClassProses }}">Siap Diisi</p>
                                @else
                                    <p class="{{ $statusClassTerkunci }}">Terkunci</p>
                                @endif
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_asesmen) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex justify-between items-start space-x-4">
                                        <div>
                                            ...
                                            @if ($level >= $level_asesmen)
                                                <p class="{{ $statusClassSelesai }}">Rekomendasi Kompeten</p>
                                            @elseif ($level >= DataSertifikasiAsesi::STATUS_ASESMEN_PRAKTEK_SELESAI)
                                                <p class="{{ $statusClassProses }}">Menunggu Jadwal Asesor</p>
                                            @else
                                                <p class="{{ $statusClassTerkunci }}">Terkunci</p>
                                            @endif
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_umpan_balik) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                ...
                                {{-- Status Dinamis --}}
                                @if ($level >= $level_umpan_balik)
                                    <p class="{{ $statusClassSelesai }}">Selesai</p>
                                @elseif ($level == $level_asesmen)
                                    <p class="{{ $statusClassProses }}">Siap Diisi</p>
                                @else
                                    <p class="{{ $statusClassTerkunci }}">Terkunci</p>
                                @endif
                        ...
                        <li class="relative flex items-start pb-10">
                            ...
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= DataSertifikasiAsesi::STATUS_BANDING_SELESAI) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                ...
                                {{-- Status Dinamis --}}
                                @if ($level >= DataSertifikasiAsesi::STATUS_BANDING_SELESAI)
                                    <p class="{{ $statusClassSelesai }}">Banding Selesai</p>
                                @elseif ($level >= $level_umpan_balik)
                        ...
                        <li class="relative flex items-start">
                            <div class="relative flex-shrink-0 mr-6">
                                ...
                                @if ($level >= $level_rekomendasi) {!! renderCheckmark() !!} @endif
                            </div>
                            <div class="flex-1">
                                <h3 class="{{ $linkClassDisabled }}">Keputusan Komite</h3>
                                
                                {{-- Status Dinamis --}}
                                @if ($level == $level_rekomendasi)
                                    <p class="{{ $statusClassSelesai }}">Kompeten - Direkomendasikan Menerima Sertifikat</p>
                                    <button class="mt-2 px-4 py-1.5 bg-green-500 text-white text-xs font-semibold rounded-md hover:bg-green-600">
                                        Unduh Sertifikat
                                    </button>
            ...
            <main class="flex-1 p-10 overflow-y-auto">
                <div class="max-w-3xl mx-auto">
                    <div class="alert alert-info text-center shadow-sm bg-white p-10 rounded-2xl"
                        ...
                        <a href="/" class="mt-6 inline-block px-6 py-3 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-600">
                            Lihat Daftar Skema
                        </a>
                    </div>
                </div>
