...
<head>
    ...
    <style>
        ...
        /* Timer Warning Animation (Merah Kedip) */
        @keyframes pulse-red {
...
<body class="bg-gray-100 min-h-screen font-sans antialiased text-slate-800">
    ...
    <div id="modal-confirm" class="fixed inset-0 z-[60] hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity opacity-0" id="confirm-backdrop">
        </div>
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg scale-95 opacity-0"
                    id="confirm-panel">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
                        <button type="button" id="btn-confirm-yes"
                            class="inline-flex w-full justify-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 sm:w-auto transition-all duration-200">
                            Ya, Lanjutkan
                        </button>
                        <button type="button" id="btn-confirm-no"
                            class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto transition-all duration-200">
                            Batal
                        </button>
                    </div>
                </div>
    ...
    <div id="modal-alert" class="fixed inset-0 z-[70] hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity opacity-0" id="alert-backdrop">
        </div>
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md scale-95 opacity-0"
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div id="alert-icon-bg"
                                ...
                                {{-- Icon Sukses --}}
                                <svg id="icon-success" class="h-6 w-6 text-green-600 hidden" fill="none"
                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                                </svg>
                                {{-- Icon Error --}}
                                <svg id="icon-error" class="h-6 w-6 text-red-600 hidden" fill="none"
                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                                </svg>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <button type="button" id="btn-alert-ok"
    ...
    <div id="timeout-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border-t-8 border-red-500 animate-bounce-in">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-xl leading-6 font-bold text-gray-900" id="modal-title">Waktu Habis!</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Waktu pengerjaan telah berakhir. Sistem sedang <strong>mengirimkan jawaban Anda
                                        secara otomatis</strong>.
                                </p>
                            </div>
                            <div
                                class="mt-4 flex justify-center sm:justify-start items-center space-x-2 text-blue-600 font-semibold animate-pulse">
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                            ...
                            {{-- TIMER --}}
                            <div id="timer-box"
                                class="bg-white border-2 border-gray-200 text-gray-800 px-5 py-2 rounded-lg font-mono font-bold flex items-center shadow-sm transition-colors duration-300">
                        ...
                        <div class="flex-1 lg:w-3/4">
                            ...
                            <div id="question-container" class="hidden">
                                ...
                                <div class="flex justify-between items-center mt-6 font-medium">
                                    <button id="btn-prev"
                                        class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition disabled:opacity-50 flex items-center shadow-sm">
                                        Sebelumnya
                                    </button>
                                    <button id="btn-next"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // === 1. FUNGSI HELPER POPUP (ANIMASI HALUS) ===
            ...
            // Custom Confirm (Return Promise biar bisa di-await)
            function showConfirm(title, message, isWarning = false) {
                return new Promise((resolve) => {
                    ...
                    if (isWarning) {
                        dom.confirmIconBg.className =
                            "mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-yellow-100 sm:mx-0 sm:h-10 sm:w-10";
                        dom.confirmIconBg.querySelector('svg').className = "h-6 w-6 text-yellow-600";
                    } else {
                        dom.confirmIconBg.className =
                            "mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-blue-100 sm:mx-0 sm:h-10 sm:w-10";
                        dom.confirmIconBg.querySelector('svg').className = "h-6 w-6 text-blue-600";
                    }
            ...
            function showAlert(type, title, message, callback = null) {
                ...
                if (type === 'success') {
                    ...
                    dom.alertIconBg.className =
                        "mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-green-100 sm:mx-0 sm:h-10 sm:w-10";
                    dom.btnAlertOk.className =
                        "inline-flex w-full justify-center rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500 sm:ml-3 sm:w-auto transition-colors";
                } else {
                    ...
                    dom.btnAlertOk.className =
                        "inline-flex w-full justify-center rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto transition-colors";
                }
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        questionsData.forEach(q => {
                            if (q.jawaban_tersimpan) userAnswers[q.id_lembar_jawab] = q
                                .jawaban_tersimpan;
                        });
                        initApp();
                    } else {
            ...
            function loadQuestion(index) {
                ...
                dom.optionsBox.innerHTML = data.opsi.map(opsi => {
                    ...
                    return `
                        <label class="option-label flex items-center p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition w-full bg-white shadow-sm relative overflow-hidden group">
                            ...
                            <div class="option-key w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-lg border border-gray-300 text-gray-500 font-bold mr-4 uppercase transition-transform group-hover:scale-110">${opsi.key}</div>
            ...
            function updateNavGridStatus() {
                document.querySelectorAll('.nav-btn-item').forEach(btn => {
                    ...
                    if (questionsData[idx]) {
                        ...
                        btn.className =
                            `nav-btn-item w-full aspect-square rounded-lg border text-sm font-medium transition focus:outline-none `;
            ...
            // === 3. LOGIC SUBMIT (SUDAH DIPERBAIKI TOMBOL OK-NYA) ===
            async function submitAssessment(isAuto = false) {
                
                // A. Konfirmasi Pop-Up (Cuma buat Manual)
                if (!isAuto) {
                    ...
                    if (answered < total) {
                        const yakin = await showConfirm(
                            ...
                            `Anda baru menjawab <strong>${answered} dari ${total}</strong> soal.<br>Soal kosong dianggap salah.<br><br>Yakin ingin mengumpulkan?`,
                            true 
                        );
                        if (!yakin) return;
                    } else {
                        const yakin = await showConfirm(
                            'Konfirmasi Selesai',
                            'Pastikan jawaban sudah benar. Data tidak bisa diubah setelah dikirim.',
                            false 
                        );
                        if (!yakin) return;
                    }
                ...
                const payload = questionsData.map(q => {
                    let ans = userAnswers[q.id_lembar_jawab];
                    if (!ans) ans = null; // Kirim null kalau kosong
                    return {
                ...
                try {
                    ...
                    if (response.ok && result.success) {
                        ...
                        // [FIX] BUKA KUNCI LAYAR (PENTING BANGET INI)
                        document.body.style.pointerEvents = 'auto'; 

                        // 1. Sembunyikan modal merah
                        if (isAuto) {
                    ...
                    // [FIX] BUKA KUNCI LAYAR JUGA KALAU ERROR
                    document.body.style.pointerEvents = 'auto'; 

                    if(isAuto) {
                        dom.modalTimeout.classList.add('hidden');
                        showAlert('error', 'Gagal Auto-Submit', "Waktu habis, tapi gagal kirim: " + error.message + ". Silakan Screenshot jawaban Anda!");
                    } else {
