        ...
        <main class="flex-1 bg-white p-8 lg:p-12 overflow-y-auto ml-0 main-content-shifted">
            ...
            <div class="max-w-6xl mx-auto content-font-base">
                ...
                @php
                    ...
                    // Data Statis untuk Pertanyaan dan Bukti (digunakan jika DB kosong)
                    $pertanyaan_static = [
                    ...
                    // Mengambil data IA09/Bukti Dasar, memastikan ada 4 record untuk loop.
                    $bukti_portofolio_data = $bukti_portofolio_data ?? collect([]);
                    $pertanyaan_ia09_data = $pertanyaan_ia09_data ?? collect([]);
                    
                    // Membuat data terintegrasi (merged_data)
                    // Jumlah loop ditentukan oleh jumlah item di bukti_portofolio_data (dinamis)
                    $merged_data = $bukti_portofolio_data->map(function($bukti, $index) use ($pertanyaan_ia09_data, $pertanyaan_static) {
                        $id_input = $bukti->id_bukti_dasar;
                        
                        // Cari record kesimpulan IA09 yang sudah ada
                        $ia09_record = $pertanyaan_ia09_data->where('id_portofolio', $id_input)->first();
                        $nomor = $index + 1;

                        return (object)[
                            'id_input' => $id_input, // ID yang kita gunakan di input (ID Bukti Dasar)
                            'bukti_dasar' => $bukti->bukti_dasar,
                            'pertanyaan_teks' => $pertanyaan_static[$nomor] ?? 'Pertanyaan Default Statis.',
                            'kesimpulan_jawaban_asesi' => $ia09_record->kesimpulan_jawaban_asesi ?? '',
                        ];
                    });
                    
                    // FALLBACK jika DB Bukti Dasar kosong (hanya 4 baris dummy)
                    if ($merged_data->isEmpty()) {
                         $merged_data = collect(range(1, 4))->map(function($i) use ($pertanyaan_static, $bukti_static) {
                            return (object)[
                                'id_input' => $i,
                                'bukti_dasar' => $bukti_static[$i] ?? 'N/A',
                                'pertanyaan_teks' => $pertanyaan_static[$i] ?? 'Pertanyaan Default Statis.',
                ...
                <div class="mb-8 text-center"> 
                    ...
                    <h1 class="text-2xl font-bold text-black uppercase tracking-wide border-b-2 border-gray-100 pb-4 mb-6 inline-block">
                        FR.IA.09. PW – PERTANYAN WAWANCARA
                    </h1>
                </div>
                ...
                {{-- 2A. TABEL BUKTI PORTOFOLIO (JUMLAH BARIS DINAMIS) --}}
                <div class="mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full table-auto table-formal text-sm min-w-[500px]">
                            ...
                            <tbody>
                                {{-- Loop Data Bukti Portofolio (Menggunakan merged_data - jumlah dinamis) --}}
                                @forelse ($merged_data as $index => $item)
                                <tr class="bg-white hover:bg-gray-50">
                                    ...
                                    <td class="p-2 align-top border-gray-300">
                                        {{-- Menampilkan bukti_dasar --}}
                                        <textarea rows="2" name="bukti_portofolio_{{ $item->id_input }}" placeholder="Judul Bukti Portofolio" readonly>{{ $item->bukti_dasar }}</textarea>
                                    </td>
                                </tr>
                                @empty
                                <tr class="bg-white hover:bg-gray-50">
                                    <td colspan="2" class="p-4 text-center text-gray-500">
                                        Data Bukti Portofolio tidak ditemukan.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
                ...
                {{-- 2B. TABEL DAFTAR PERTANYAAN WAWANCARA (Kesimpulan Jawaban Asesi Terhubung DB) --}}
                <div class="mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full table-auto table-formal text-sm min-w-[900px]">
                            ...
                            <tbody>
                                {{-- Loop menggunakan data yang sudah digabungkan (jumlah dinamis) --}}
                                @forelse ($merged_data as $index => $item)
                                <tr class="bg-white hover:bg-gray-50">
                                    <td class="p-2 text-center align-top border-gray-300 font-bold" style="width: 5%;">{{ $index + 1 }}.</td>
                                    <td class="p-2 align-top border-gray-300">
                                        {{-- DAFTAR PERTANYAAN WAWANCARA (STATIS) --}}
                                        <textarea rows="4" name="pertanyaan_{{ $item->id_input }}" placeholder="Pertanyaan... (Asesor mengisi/memodifikasi)" readonly>{{ $item->pertanyaan_teks }}</textarea>
                                    </td>
                                    <td class="p-2 align-top border-gray-300">
                                        {{-- KESIMPULAN JAWABAN ASESI (Input yang terhubung ke DB) --}}
                                        {{-- Menggunakan id_input (id_bukti_dasar) sebagai key untuk POST data --}}
                                        <textarea rows="4" name="kesimpulan[{{ $item->id_input }}]" placeholder="Kesimpulan Asesor atas jawaban Asesi">{{ $item->kesimpulan_jawaban_asesi }}</textarea>
                                    </td>
                                </tr>
                                @empty
                                <tr class="bg-white hover:bg-gray-50">
                                    <td colspan="3" class="p-4 text-center text-gray-500">
                                        Data Pertanyaan Wawancara tidak dapat dimuat (Bukti Portofolio kosong).
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
