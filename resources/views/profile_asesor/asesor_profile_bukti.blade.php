...
<head>
  ...
  {{-- Menggunakan Font Awesome terbaru untuk konsistensi --}}
...
<body class="text-gray-800">
  ...
  {{-- Layout Utama: Menggunakan Flex seperti Settings --}}
  <div class="flex min-h-[calc(100vh-80px)]">
    ...
    <main class="ml-[22%] h-[calc(100vh-80px)] overflow-y-auto p-8 bg-gray-50 flex-1">
      ...
      {{-- Kontainer Besar dengan Shadow & Padding dari Settings --}}
      <div class="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-10 border border-gray-100 min-h-full">
        ...
        <div class="mt-10 p-10 border border-gray-200 rounded-2xl shadow-md relative">
          ...
          <div class="flex flex-col items-center justify-center">
              <div class="group relative mt-2 border-2 border-dashed border-gray-300 hover:border-blue-400 rounded-xl w-full max-w-md h-48 flex items-center justify-center bg-gray-50 transition-colors duration-300">
                  ...
                  @if(!$asesor->tanda_tangan)
                  <a href="{{ route('edit_asesor3', $asesor->id_asesor) }}" class="absolute inset-0 flex items-center justify-center bg-white/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer">
        ...
        <div class="mt-8 flex flex-col sm:flex-row justify-end items-center gap-4">
            ...
            {{-- Tombol Rejected --}}
            <form action="{{ route('asesor.update_status', $asesor->id_asesor) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menolak verifikasi asesor ini?');">
                @csrf
            ...
            {{-- Tombol Accepted --}}
            <form action="{{ route('asesor.update_status', $asesor->id_asesor) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui verifikasi asesor ini?');">
                @csrf
