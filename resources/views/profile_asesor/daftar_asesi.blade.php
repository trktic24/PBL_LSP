    ...
    <main class="ml-[22%] h-[calc(100vh-80px)] overflow-y-auto p-8 bg-gray-50 flex-1">
      ...
      <div class="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-10 border border-gray-100 min-h-full">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('asesor_profile_tinjauan', $asesor->id_asesor) }}" class="flex items-center text-gray-500 hover:text-blue-600 transition text-sm font-medium">
                ...
                <tbody class="text-gray-600 text-sm font-medium">
                    @forelse($jadwal->dataSertifikasiAsesi as $index => $item)
                        <tr class="border-b border-gray-100 hover:bg-blue-50/30 transition duration-150">
                            ...
                            {{-- Status Pra Asesmen (LINK DIPERBAIKI) --}}
                            @php
                            ...
                            <td class="py-4 px-6 text-center">
                                {{-- Mengarah ke route 'asesor.assessment.detail' --}}
                                <a href="{{ route('asesor.assessment.detail', ['id_asesor' => $asesor->id_asesor, 'id_data_sertifikasi_asesi' => $item->id_data_sertifikasi_asesi]) }}" 
                                   class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold {{ $classPra }} hover:opacity-80 transition">
                                    {{ $statusPra }}
                                </a>
                            </td>

                            {{-- Status Asesmen (LINK DIPERBAIKI) --}}
                            @php
                            ...
                            @endphp
                            <td class="py-4 px-6 text-center">
                                {{-- Mengarah ke route 'asesor.assessment.detail' dengan anchor #asesmen --}}
                                <a href="{{ route('asesor.assessment.detail', ['id_asesor' => $asesor->id_asesor, 'id_data_sertifikasi_asesi' => $item->id_data_sertifikasi_asesi]) }}#asesmen" 
                            ...
                            <td class="py-4 px-6 text-center">
                                ...
                                @else
                                    <a href="{{ route('asesor.assessment.detail', ['id_asesor' => $asesor->id_asesor, 'id_data_sertifikasi_asesi' => $item->id_data_sertifikasi_asesi]) }}" class="text-yellow-600 font-bold text-xs hover:underline"><i class="fas fa-exclamation-circle mr-1"></i> Perlu Verifikasi</a>
                                @endif
                            </td>
